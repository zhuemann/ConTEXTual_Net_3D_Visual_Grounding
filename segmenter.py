    ...
    def __getitem__(self, index):
        ...
        if type(data_item) == type([1,2]):
            ...
            #print(data_item.keys())
                ...
                text = text.replace(str(data_item["slice_num"]), "")
                text = text.replace(str(data_item["suv_num"]), "")
                text = text.replace("/mnt/Bradshaw/UW_PET_Data/resampled_cropped_images_and_labels/images/", "")
...
class DataTransformBuilder:
    ...
    def get_crop_transforms(self):
        ...
        if self.crop_mode == "ratio":
            ...
            ts.append(SpatialPadd(keys = keys, spatial_size = (192, 192, None), mode = "constant", method="symmetric", constant_values=0))
            ...
            ts.append(SpatialPadd(keys=keys, spatial_size=(None, None, 352), mode="constant", method="end"))  # Pad from the end (which is the start of the original after flipping)
...
class Segmenter:
    def __init__(
        ...
        lang_model = AutoModel.from_pretrained(MODEL_NAME, output_hidden_states=True)
    ...
    def get_train_loader(self, data, tokenizer ,cache_rate=0, persistent_workers=False):
        ...
        if cache_rate > 0:
            ...
            #)
            train_ds = TextImageCacheDataset(data=data, tokenizer=tokenizer, transform=train_transform, copy_cache=False, cache_rate=cache_rate, runtime_cache=runtime_cache)
    ...
    def get_val_loader(self, data, tokenizer, cache_rate=0, resample_label=False, persistent_workers=False):
        ...
        if cache_rate > 0:
            ...
            val_ds = TextImageCacheDataset(data=data, tokenizer=tokenizer, transform=val_transform, copy_cache=False, cache_rate=cache_rate, runtime_cache=runtime_cache)
        else:
            runtime_cache = self.get_shared_memory_list(length=len(data))
    ...
    def train(self):
        ...
        #print(train_files)
        train_loader = self.get_train_loader(data=train_files, tokenizer=tokenizer, cache_rate=cache_rate_train, persistent_workers=True)
        val_loader = self.get_val_loader(
            data=validation_files, tokenizer=tokenizer, cache_rate=cache_rate_val, resample_label=True, persistent_workers=True
        )

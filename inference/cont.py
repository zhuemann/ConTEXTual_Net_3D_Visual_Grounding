...
def zero_pad_and_cat_mine(tensor1, tensor2, dim=1):
    """
    Zero-pad tensor1 and/or tensor2 along spatial dims so that all
    non-concat dimensions match, then concatenate along `dim`.

    Assumes 5D tensors of shape (N, C, D, H, W) and typically dim=1.
    """
    if tensor1.dim() != 5 or tensor2.dim() != 5:
        raise ValueError("zero_pad_and_cat assumes 5D tensors (N, C, D, H, W).")

    if tensor1.shape[0] != tensor2.shape[0]:
        raise ValueError("Batch size (dim 0) must match.")
    if dim == 1 and tensor1.shape[1] != tensor2.shape[1]:
        # For UNet we usually concat along channels, but here our skip/upsample
        # paths have the same C before concat, so we enforce it.
        pass

    n1, c1, d1, h1, w1 = tensor1.shape
    n2, c2, d2, h2, w2 = tensor2.shape

    # Target sizes for spatial dims
    target_d = max(d1, d2)
    target_h = max(h1, h2)
    target_w = max(w1, w2)

    def pad_to(t, target_d, target_h, target_w):
        _, _, d, h, w = t.shape
        pad_d = target_d - d
        pad_h = target_h - h
        pad_w = target_w - w

        # F.pad pad sequence is (W_left, W_right, H_left, H_right, D_left, D_right)
        pad = [0, max(pad_w, 0),
               0, max(pad_h, 0),
               0, max(pad_d, 0)]
        if any(pad):
    ...
    tensor1 = pad_to(tensor1, target_d, target_h, target_w)
    tensor2 = pad_to(tensor2, target_d, target_h, target_w)
...
class UnetUpBlock(nn.Module):
    ...
    def forward(self, inp, skip, lang_rep):
        ...
        out = zero_pad_and_cat_mine(out, skip, dim=1)
